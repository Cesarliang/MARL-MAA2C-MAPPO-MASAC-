...
def train(args):
    ...
    while mappo.n_episodes < MAX_EPISODES:
        ...
        if mappo.episode_done and ((mappo.n_episodes + 1) % EVAL_INTERVAL == 0):
            j = j+1
            rewards, _, _, speeds_eval,headway_dis= mappo.evaluation(env_eval, dirs['train_videos'], EVAL_EPISODES)

            rewards_mu, rewards_std = agg_double_list(rewards)
            av_speed = np.mean(speeds_eval)
            av_head_time = np.mean(headway_dis)/speed
            # print(rewards_std,speeds,av_speed,111)
            print('回合',mappo.n_episodes + 1,"平均奖励",rewards_mu,"奖励标准差",rewards_std,'平均速度',av_speed,"平均车头时距",av_head_time)
