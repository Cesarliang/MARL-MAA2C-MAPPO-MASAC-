...
# path = 'F:\新疆大学\强化学习\多智能体\\bendi\PPO-MEDIUM_0fenbu_avg.xlsx'
...
def train(args):
    ...
    while mappo.n_episodes < MAX_EPISODES:
        ...
        # headway_time = float(mappo.headway_distance[i]/speed)
        # time1 = float(mappo.epoch_steps[i])
        # print(time1)
        # print(headway_time)
        cursh_num += mappo.cursh
        ...
        if mappo.episode_done and ((mappo.n_episodes + 1) % EVAL_INTERVAL == 0):
            j = j+1
            rewards, _, _, speeds_eval,headway_dis,merging_cost= mappo.evaluation(env_eval, dirs['train_videos'], EVAL_EPISODES)
            ...
            print('回合',mappo.n_episodes + 1,"平均奖励",rewards_mu,"奖励标准差",rewards_std,'平均速度',av_speed,"平均车头时距",av_head_time,"汇入成本",av_merging_cost)
            ...
            sheet.cell(j, 12, float(cursh_num))
            sheet.cell(j,13,float(av_head_time))
            sheet.cell(j,14,float(av_merging_cost))
            workbook.save(path)
            cursh_num = 0
