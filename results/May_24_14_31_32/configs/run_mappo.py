...
def train(args):
    ...
    while mappo.n_episodes < MAX_EPISODES:
        mappo.interact()
        # print(mappo.n_episodes,"a")
        if mappo.n_episodes >= EPISODES_BEFORE_TRAIN:
            mappo.train()

        reward = mappo.episode_rewards[mappo.n_episodes-1]
        speed = mappo.average_speed[mappo.n_episodes-1]
        i = mappo.n_episodes-1
        print("回合数：",mappo.n_episodes,"当前回合奖励：",mappo.episode_rewards[mappo.n_episodes-1]
              ,"v_loss:",mappo.v_loss1,"平均速度",mappo.average_speed[i])
        rewards.append(reward)
        speeds.append(speed)
        v_losss.append(mappo.v_loss1)
        if ((i % 500) == 0 or i == 10) and i != 0:
            qidian = 0
            now = time.localtime()
            ...
            if i %500 ==0:
                qidian = i -500
            for i in tqdm(range(qidian,i)):
                time.sleep(0.1)
                sheet.cell(i+2,1,i)
                sheet.cell(i+2,2,float(rewards[i]))
                sheet.cell(i+2,4,v_losss[i])
                sheet.cell(i+2,6,speeds[i])
            workbook.save(path)
