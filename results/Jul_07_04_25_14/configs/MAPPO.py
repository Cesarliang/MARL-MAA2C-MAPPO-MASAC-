        ...
        for agent_id in range(self.n_agents):
            ...
            for param in self.actor.parameters():
                if param.grad is not None:
                    actor_gradients.append(param.grad.data.clone())
                else:
                    actor_gradients.append(None)
            ...
            actor_avg_gradients = []
        print(len(actor_gradients))
        for i in range(len(actor_gradients[0])):
            param_gradients = [grad[i] for grad in actor_gradients if grad is not None]
            avg_gradient = torch.mean(torch.stack(param_gradients), dim=0)
            actor_avg_gradients.append(avg_gradient)

        # 计算critic网络的平均梯度
        critic_avg_gradients = []
        for i in range(len(critic_gradients[0])):
            param_gradients = [grad[i] for grad in critic_gradients if grad is not None]
            avg_gradient = torch.mean(torch.stack(param_gradients), dim=0)
            critic_avg_gradients.append(avg_gradient)
