...
path = 'F:\新疆大学\强化学习\联邦元强化\\测试用.xlsx'
...
def train(args):
    ...
    print("路径", path, "模式", mappo.traffic_density, "seeds", env.seed,"车辆数",mappo.n_agents)
    ...
    while mappo.n_episodes < MAX_EPISODES:
        ...
        print("回合数：", i, "当前回合奖励：", reward, "平均速度", speed,"车辆数",mappo.n_agents)
        ...
        if mappo.episode_done and ((mappo.n_episodes + 1) % EVAL_INTERVAL == 0):
            j = j + 1
            rewards, _, _, speeds_eval, headway_dis, merging_cost = mappo.evaluation(env_eval, dirs['train_videos'],
                                                                                     EVAL_EPISODES)
