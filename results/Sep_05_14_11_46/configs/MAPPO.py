...
class MAPPO:
    ...
    def compute_grad_actor(
        ...
        model: torch.nn.Module,
            states_var, actions_var, actor_target, advantages,
        ...
        loss_1 = self.actor_loss(model,states_var, actions_var, actor_target, advantages)
        ...
        loss_2 = self.actor_loss(model,states_var,actions_var,actor_target,advantages)
    ...
    def compute_grad_critic(
        ...
        model.load_state_dict(dummy_model_params_1, strict=False)
        loss_1 = self.critic_loss(model,states_var, actions_var, target_values)
        # loss_1 = self.criterion(logit_1, y)
        grads_1 = torch.autograd.grad(loss_1, model.parameters())
        ...
        # logit_2 = model(x)
        # loss_2 = self.criterion(logit_2, y)
        loss_2 = self.critic_loss(model,states_var,actions_var,target_values)
        grads_2 = torch.autograd.grad(loss_2, model.parameters())
        ...
        with torch.no_grad():
            for g1, g2 in zip(grads_1, grads_2):
                grads.append((g1 - g2) / (2 * delta))
            return grads
    def actor_loss(self,actor,states_var,actions_var,actor_target,advantages):



        action_log_probs = actor(states_var)
        action_log_probs = th.sum(action_log_probs * actions_var, 1)
        old_action_log_probs = actor_target(states_var).detach()
    ...
    def critic_lossc(self, critic, states_var, actions_var, target_values):
        ...
        values = critic(states_var, actions_var)
    ...
    def train(self):
        ...
        for agent_id in range(self.n_agents):
            ...
            temp_actor_loss= self.actor_loss(temp_actor,states_var[:, agent_id, :],actions_var[:, agent_id, :],self.actor_targets[agent_id],advantages)
            ...
            grads_2nd = self.compute_grad_actor(
                self.actors[agent_id], states_var[:, agent_id, :],actions_var[:, agent_id, :],self.actor_targets[agent_id],advantages,v=grads_1st)
            ...
            for param, grad1, grad2 in zip(
                ...
                param.data.sub_(0.001 * grad1 - 0.001 * 0.01 * grad2)
            '///'
            #
            ...
            temp_critic_loss = self.critic_lossc(temp_critic, states_var[:, agent_id, :], actions_var[:, agent_id, :],
                                              target_values)
            ...
            critic_loss_1st = self.critic_lossc(temp_critic, states_var[:, agent_id, :], actions_var[:, agent_id, :],
                                             target_values)
            ...
            grads_2nd = self.compute_grad_critic(
                ...
                self.critic_targets[agent_id], advantages, v=grads_1st)
            # 获取第三个数据批次 data_batch_3，并使用临时模型、第二次梯度 grads_1st 以及参数 v 和 second_order_grads 执行二阶梯度计算，得到梯度 grads_2nd。
            # NOTE: Go check https://github.com/KarhouTam/Per-FedAvg/issues/2 if you confuse about the model update.
            for param, grad1, grad2 in zip(
