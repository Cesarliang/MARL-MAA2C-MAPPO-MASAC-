...
class MergeEnv(AbstractEnv):
    ...
    def _reward(self, action: int) -> float:
        ...
        for vehicle in self.controlled_vehicles:
            reward, cursh,headway_dis ,merging_cost= self._agent_reward(action, vehicle)
            reward1 = reward + reward1
        reward2 = reward1 / len(self.controlled_vehicles)
        return reward2
    ...
    def _agent_reward(self, action: int, vehicle: Vehicle) -> float:
       ...
       """
        ...
        cursh_reward = self.config["COLLISION_REWARD"] * (-1 * vehicle.crashed)
        ...
        return reward, cursh_reward, headway_distance , Merging_lane_cost
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, dict]:
        ...
        for vehicle in self.controlled_vehicles:
            vehicle.local_reward, vehicle.cursh_reward, vehicle.headway_distance, vehicle.merging_cost= self._agent_reward(action, vehicle)
        info["cursh_reward"] = tuple(vehicle.cursh_reward for vehicle in self.controlled_vehicles)
        info["headway_distance"] = tuple(vehicle.headway_distance for vehicle in self.controlled_vehicles)
        info["merging_cost"] = tuple(vehicle.merging_cost for vehicle in self.controlled_vehicles)
        # local reward
    ...
    def _reset(self, num_CAV=0) -> None:
        ...
        if self.config["traffic_density"] == 1:#修改车辆数
            # easy mode: 1-3 CAVs + 1-3 HDVs
            if num_CAV == 0:
                num_CAV = np.random.choice(np.arange(1, 2), 1)[0]
            else:
                num_CAV = num_CAV
            ...
            if num_CAV == 0:
                num_CAV = np.random.choice(np.arange(2, 3), 1)[0]
            else:
                num_CAV = num_CAV
            num_HDV = np.random.choice(np.arange(2, 4), 1)[0]

        elif self.config["traffic_density"] == 3:
            # hard mode: 4-6 CAVs + 3-5 HDVs
            if num_CAV == 0:
                num_CAV = np.random.choice(np.arange(3, 4), 1)[0]
            else:
                num_CAV = num_CAV
