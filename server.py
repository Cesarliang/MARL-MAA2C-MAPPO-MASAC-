...
from single_agent.Memory_commonFED import OnPolicyReplayMemory
from single_agent.Model_commonFED import ActorNetwork, CriticNetwork
...
class SERVER:
    ...
    def Integration(self,surra,surrb,target_values1,values1):

        self.surr1 = np.mean(surra)
        self.surr2 = np.mean(surrb)
        self.target_values = np.mean(target_values1)
        self.value = np.mean(values1)
    ...
    def update(self):
        ...
        # surr1 = surra[agent_id]
        # surr2 = surrb[agent_id]

        actor_loss = -th.mean(th.min(self.surr1, self.surr2))
        actor_loss.backward()
        if self.max_grad_norm is not None:
            nn.utils.clip_grad_norm_(self.actor.parameters(), self.max_grad_norm)
        self.actor_optimizer.step()
        # ...............critic
        # target_values = target_values1[agent_id]
        # values = values1[agent_id]
        self.critic_optimizer.zero_grad()
        if self.critic_loss == "huber":
            critic_loss = nn.functional.smooth_l1_loss(self.values, self.target_values)
        else:
            critic_loss = nn.MSELoss()(self.values, self.target_values)
